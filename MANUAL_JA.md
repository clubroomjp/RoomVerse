...
4.  **辞書 (Lorebook)**:
    *   **辞書マネージャー**: Character Settingsにある「辞書 (Lorebook)」ボタンから、専用の管理画面を開けます。
    *   **複数辞書の管理**: 複数の辞書を作成・切り替え可能です。「使用する (Set Current)」ボタンで、チャットで有効にする辞書を選択します。
    *   **インポート**: SillyTavern形式のJSONファイルをインポートボタン（ファイルアイコン）から取り込めます。
    *   **編集とリネーム**:
        *   鉛筆アイコンで辞書名を変更できます。
        *   エントリの「キーワード」を変更して保存すると、自動的に「古いエントリを削除して新規作成（リネーム）」する確認が出ます。
